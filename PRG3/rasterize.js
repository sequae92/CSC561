...
var vTextureCoordAttribLoc; // where to put position for texture coordinate
...
/*
function newModel(index, dist, alpha, type) {
    this.index = index;
    this.dist = dist;
    this.alpha = alpha;
    this.type = type;
...
function handleKeyDown(event) {
    ...
    switch (event.code) {
        ...
        case "Backspace": // reset model transforms to default
            ...
            break;
        case "KeyB":    // in the case of blending one model to other
...
// custom function to find if value is power of 2
function isPowerOf2(value) {
    return (value & (value - 1)) == 0;
}

//function to set the parameters of created texture object
function handleLoadedTexture(texture) {
    ...
    //texture.isLoaded = true; //assign the value to isLoaded attribute after loading texture
    gl.bindTexture(gl.TEXTURE_2D, null);

}

//function to initialise the texture object
function initTexture(imageSrc){
    var texture = gl.createTexture();
    texture.image = new Image();
    texture.image.crossOrigin = "Anonymous";
    texture.image.src = "https://ncsucgclass.github.io/prog3/" + imageSrc;
    ...
    function makeEllipsoid(currEllipsoid,numLongSteps) {
        ...
        try {
            if (numLongSteps % 2 != 0)
                ...
                var ellipsoidTextureCoords = [1,0]; // this is added for south pole, but why? 

                for (var latAngle=-latLimitAngle; latAngle<=latLimitAngle; latAngle+=angleIncr) {
                    latRadius = Math.cos(latAngle); // radius of current latitude
                    latY = Math.sin(latAngle); // height at current latitude
                    for (var longAngle=0; longAngle<2*Math.PI+angleIncr; longAngle+=angleIncr)
                    { // for each long
                ...
                ellipsoidTextureCoords.push(0,1); // this is added for north pole, but why ?
            ...
            return({vertices:ellipsoidVertices, normals:ellipsoidNormals, textureCrd: ellipsoidTextureCoords, triangles:ellipsoidTriangles});
        } // end try
    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                inputTriangles[whichSet].glTextureCoords = []; // flat normal list for webgl
                inputTriangles[whichSet].textureImage = inputTriangles[whichSet].material.texture;
                inputTriangles[whichSet].texture = initTexture(inputTriangles[whichSet].textureImage);
                ...
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    ...
                    textToAdd = inputTriangles[whichSet].uvs[whichSetVert]; // get uvs to add mentioned in the json file

                    inputTriangles[whichSet].glVertices.push(vtxToAdd[0],vtxToAdd[1],vtxToAdd[2]); // put coords in set coord list
                    inputTriangles[whichSet].glNormals.push(normToAdd[0],normToAdd[1],normToAdd[2]); // put normal in set coord list
                    inputTriangles[whichSet].glTextureCoords.push(textToAdd[0],textToAdd[1],textToAdd[2]);
                ...
                //buffers for handling the texture coordinates
            ...
            if (inputEllipsoids == String.null)
                ...
                for (var whichEllipsoid=0; whichEllipsoid<numEllipsoids; whichEllipsoid++) {
                    ...
                    //passing the textureObject here
                    inputEllipsoids[whichEllipsoid].textureObject = initTexture(ellipsoid.texture);
                    ...
                    gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(ellipsoidModel.textureCrd),gl.STATIC_DRAW); // data in
...
function setupShaders() {
    ...
    var vShaderCode = `
        ...
        attribute vec2 aTextureCoord; //for handling texture in vertex shader
        varying vec2 vTextureCoord; // for handling texture
    ...
    var fShaderCode = `
        ...
        void main(void) {
            ...
            if(uBlendModel){
                gl_FragColor = vec4(colorOut * textureColor.rgb, uAlpha * textureColor.a);
            }
            else
                gl_FragColor = vec4(textureColor.rgb,textureColor.rgb * uAlpha);
        }
    `;
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                textBlendModel = gl.getUniformLocation(shaderProgram, "uBlendModel");
...
function renderModels() {
    ...
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
        ...
        //for textures binding buffers
        gl.bindBuffer(gl.ARRAY_BUFFER,vertexTextureCoordBuffer[whichTriSet]); // activate
        gl.vertexAttribPointer(vTextureCoordAttribLoc,3,gl.FLOAT,false,0,0); // feed
        ...
        // console.log(blendModel);
        gl.uniform1i(textBlendModel, blendModel);
        if (currSet.material.alpha) {
        ...
        gl.uniform1f(alphaUniform, parseFloat(currSet.material.alpha));
    ...
    for (var whichEllipsoid=0; whichEllipsoid<numEllipsoids; whichEllipsoid++) {
        ...
        //for textures binding buffers
        gl.bindBuffer(gl.ARRAY_BUFFER,vertexTextureCoordBuffer[numTriangleSets+whichEllipsoid]); // activate
        ...
        gl.uniform1f(alphaUniform, parseFloat(ellipsoid.alpha));
